...
import { GRID_ROWS, GRID_COLS, INITIAL_TILE_VALUES, INITIAL_TILE_WEIGHTS, TILE_COOLDOWN_TURNS } from '../constants';
import { Tile, Position, Grid } from '../types';

let nextIdCounter = 0;
export const generateId = (): string => `tile-${nextIdCounter++}`;

export const createNewTile = (r: number, c: number, value?: number): Tile => {
...
export const isValidCoordinate = (r: number, c: number): boolean => {
  return r >= 0 && r < GRID_ROWS && c >= 0 && c < GRID_COLS;
};

export const validatePath = (grid: Grid, path: Position[]): boolean => {
  if (path.length < 2) return false;

  const firstTile = grid[path[0].r][path[0].c];
  if (!firstTile) return false; // Should not happen if path is from selected tiles
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    if (visited.has(posKey)) return false; // No repeats
    visited.add(posKey);
  }
  return true;
};

export const applyGravityAndSpawn = (currentGrid: Grid): Grid => {
  const newGrid = JSON.parse(JSON.stringify(currentGrid)) as Grid; // Deep copy
...
export const hasAnyValidMerge = (grid: Grid): boolean => {
  for (let r = 0; r < GRID_ROWS; r++) {
    for (let c = 0; c < GRID_COLS; c++) {
      const tile = grid[r][c];
...
export const processMerge = (currentGrid: Grid, path: Position[], isDoublerActive: boolean): { newGrid: Grid; mergedValue: number; scoreEarned: number } => {
  if (path.length === 0) throw new Error("Path cannot be empty for merge");

  const newGrid = JSON.parse(JSON.stringify(currentGrid)) as Grid;
